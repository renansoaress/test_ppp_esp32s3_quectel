...
enum class ModemType {
    ...
    EG91,             /*!< Derived from the GenericModule with specifics applied to EG91 model */
};
...
public:
    ...
    std::shared_ptr<GenericModule> build_shared_module(const config *cfg, Args &&... args)
    {
        switch (m) {
        case ModemType::EG91:
            return build_shared_module<EG91>(cfg, std::forward<Args>(args)...);
    ...
    std::unique_ptr<DCE> build_unique(const config *cfg, Args &&... args)
    {
        switch (m) {
        case ModemType::EG91:
            return build_unique<EG91>(cfg, std::forward<Args>(args)...);
        case ModemType::SIM800:
    ...
    DCE *build(const config *cfg, Args &&... args)
    {
        switch (m) {
        case ModemType::EG91:
            return build<EG91>(cfg, std::forward<Args>(args)...);
        case ModemType::SIM800:
