...
command_result power_down_eg91(CommandableIf *t)
{
    ESP_LOGV(TAG, "%s", __func__);
    return generic_command(t, "AT+QPOWD=0\r", "POWERED DOWN", "ERROR", 3000);
}
...
command_result set_data_mode_eg91(CommandableIf *t)
{
    ESP_LOGV(TAG, "%s", __func__ );
    return generic_command(t, "ATD*99#\r", "CONNECT", "ERROR", 5000);
}
...
{
    ESP_LOGV(TAG, "%s", __func__ );
    // CMUX* (+CMUX: 0,0,5,127,10,3,30,10,2)
    // return generic_command_common(t, "AT+CMUX=0\r");
    return generic_command_common(t, "AT+CMUX=0,0,5,500,10,3,30,10,2\r");
}
...
command_result get_gps_location(CommandableIf *t, float &date, float &utc, float &latitude, float &longitude, float &altitude,float &spkm, float &hdop, int &nsat)
{
    ESP_LOGV(TAG, "%s", __func__ );
    std::string_view out;
    auto ret = generic_get_string(t, "AT+QGPSLOC=2\r", out, 500);
    if (ret != command_result::OK) {
        return ret;
    }

    constexpr std::string_view pattern = "+QGPSLOC: ";
    constexpr int ignore = pattern.size();

    std::vector<std::string_view> fields;

    // Encontrar cada campo separado por ","
    size_t start = ignore;
    while (start != std::string_view::npos && start < out.size()) {
        size_t end = out.find(',', start);
        fields.push_back(out.substr(start, end - start));
        start = end != std::string_view::npos ? end + 1 : std::string_view::npos;
    }

    // Verificar se o número de campos é o esperado
    if (fields.size() != 11) {
        std::cerr << "Número de campos inválido\n";
        return command_result::FAIL;
    }

    utc = std::stof(std::string(fields[0]));
    latitude = std::stof(std::string(fields[1]));
    longitude = std::stof(std::string(fields[2]));
    hdop = std::stof(std::string(fields[3]));
    altitude = std::stof(std::string(fields[4]));
    // fix = std::stof(std::string(fields[5]));
    // cog = std::stof(std::string(fields[6]));
    spkm = std::stof(std::string(fields[7]));
    // spkn = std::stof(std::string(fields[8]));
    date = std::stof(std::string(fields[9]));
    nsat = std::stof(std::string(fields[10]));
