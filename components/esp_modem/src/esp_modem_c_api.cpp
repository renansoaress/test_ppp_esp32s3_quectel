...
#define ESP_MODEM_C_API_STR_MAX 500
#endif
...
extern "C" esp_err_t esp_modem_at(esp_modem_dce_t *dce_wrap, const char *at, char *p_out, int timeout)
{
    ...
    if ((p_out != NULL) && (!out.empty())) {
        strncpy(p_out, out.c_str(), ESP_MODEM_C_API_STR_MAX);
    }
    return ret;
}
...
extern "C" esp_err_t esp_modem_get_gps_location(esp_modem_dce_t *dce_wrap, float *date, float *utc, float *latitude, float *longitude, float *altitude, float *spkm, float *hdop, int *nsat)
{
    if (dce_wrap == nullptr || dce_wrap->dce == nullptr) {
        return ESP_ERR_INVALID_ARG;
    }
    return command_response_to_esp_err(dce_wrap->dce->get_gps_location(*date, *utc, *latitude, *longitude, *altitude,*spkm, *hdop, *nsat));
}
